...
$landingExampleURL = "https://mbasic.facebook.com/ousama.info";
/****************************** END CONFIGURATION ******************************/
ob_start("ob_gzhandler");
if (version_compare(PHP_VERSION, "5.4.7", "<")) {
  die("oussama info requires PHP version 5.4.7 or later.");
}
$requiredExtensions = ['curl', 'mbstring', 'xml'];
foreach($requiredExtensions as $requiredExtension) {
  if (!extension_loaded($requiredExtension)) {
    die("oussama info requires PHP's \"" . $requiredExtension . "\" extension. Please install/enable it on your server and try again.");
  }
}
...
function makeRequest($url) {
  ...
  if (empty($user_agent)) {
    $user_agent = "Mozilla/5.0 (Linux; U; Android 2.3.5; en-us; HTC Vision Build/GRI40)(compatible; oussama info)";
  }
  $ch = curl_init();
  ...
  switch ($_SERVER["REQUEST_METHOD"]) {
    case "POST":
      ...
      if (isset($postData["oussamaFormAction"])) {
        unset($postData["oussamaFormAction"]);
      }
...
if (isset($_POST["oussamaFormAction"])) {
  $url = $_POST["oussamaFormAction"];
  unset($_POST["oussamaFormAction"]);
} else {
  $queryParams = Array();
  ...
  if (isset($queryParams["oussamaFormAction"])) {
    $formAction = $queryParams["oussamaFormAction"];
    unset($queryParams["oussamaFormAction"]);
...
if (empty($scheme)) {
  ...
  if (strpos($url, "//") === 0) {
    ...
    die('Error: Detected a "' . $scheme . '" URL. oussama exclusively supports http[s] URLs.');
}
...
if (stripos($contentType, "text/html") !== false) {
  ...
  foreach($xpath->query("//form") as $form) {
    ...
    $actionInput->appendXML('<input type="hidden" name="oussamaFormAction" value="' . htmlspecialchars($action) . '" />');
  ...
  echo "<!-- Proxified page constructed by oussama -->\n" . $doc->saveHTML();
